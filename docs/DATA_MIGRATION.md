...
管理画面の初期化機能は、schemaを作り直すmigrationではありません。全注文と管理者以外の参加者を削除し、唯一の管理者、メニュー、操作履歴、migration情報を保持します。参加者・担当者の削除時は外部キー制約により、過去の操作履歴にある実行者との紐づけが解除され、画面では「システム」と表示されます。新しい列やtableは使わないため、追加migrationは不要です。
