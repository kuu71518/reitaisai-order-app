...
初回管理者用に、自分のDiscord数値User IDも控えます。DiscordのDeveloper Modeを有効にし、自分のプロフィールから`Copy User ID`を選びます。この値も公開文書へ書きません。参加者とmanagerのUser IDは、本人確認後に管理画面へ1件ずつ事前登録します。

ログイン許可照合用の`DISCORD_ID_HMAC_KEY`も環境ごとに作ります。パスワード管理アプリの生成機能で32文字以上のランダム値をstaging用・production用に別々に作り、同アプリへ保存します。この値は継続運用に必要で、変更すると全利用者の再登録が必要です。
...
npx wrangler secret put DISCORD_ID_HMAC_KEY --env staging --config wrangler.deploy.toml
npx wrangler secret put BOOTSTRAP_ADMIN_DISCORD_USER_ID --env staging --config wrangler.deploy.toml
Pop-Location
```

Secret入力時は値を画面共有・録画・チャットへ出しません。`DISCORD_ID_HMAC_KEY`は削除せず、bootstrap IDだけを初回管理者ログイン後に削除します。Workerの`Settings > Domains & Routes`でstaging API custom domainが有効になったことを確認します。
...
3. staging fixtureの唯一の未登録adminへbootstrapされることを確認する。
...
5. 管理画面で本人確認済みの試験用Discord User IDをmemberへ登録する。
6. 登録済みアカウントはログインでき、未登録アカウントは拒否されることを確認する。
7. memberから宴会コースが見えず、URL/APIへ商品IDを直接送っても注文できないことを確認する。
8. adminから宴会コースが見え、利用者へ代理追加すると即「注文済み」になり、本人履歴へ「管理者が事前に追加」と出ることを確認する。
9. 誤って事前追加した注文だけを訂正でき、本人履歴・合計・監査ログへ反映されることを確認する。
10. memberが管理APIへ入れないことを確認する。
11. managerが自グループ以外を見られないことを確認する。
12. 注文、二重送信防止、数量変更、伝達済み、会計、ログアウトを確認する。
13. JST 21:29では深夜料金注意が出ず、21:30では表示されること、追加toastと固定カートを320px幅で確認する。
14. iPhone SafariとAndroid Chromeの少なくとも1台ずつで確認する。
15. 同じ会場Wi-Fiから複数端末で通常ログインし、WAFの誤検知がないことを確認する。
16. WAF rule名、対象path、閾値、期間、action、検証結果をrelease記録へ残す。
...
npx wrangler secret put DISCORD_ID_HMAC_KEY --env production --config wrangler.deploy.toml
npx wrangler secret put BOOTSTRAP_ADMIN_DISCORD_USER_ID --env production --config wrangler.deploy.toml
Pop-Location
```

`db:bootstrap:production`は初回の新規production D1専用で、空D1を再検査してからbaseline migrationを適用します。2回目のmigration一覧（未適用0件）、`db:seed:production`の静的検査と投入まで、すべて成功することを確認します。seed検査はplaceholderや例示値を拒否し、`users`/`menu_items`へのVALUES形式INSERTだけ、有効な初期adminアカウントちょうど1人、menu 1件以上、宴会コースの`is_admin_only = 1`を要求します。投入後のuser/menu件数が準備資料と一致することも確認します。
...
既存D1へ`0003_discord_allowlist_and_admin_orders.sql`を適用する場合は例外的にメンテナンスが必要です。旧Discord登録と全sessionが消え、旧Workerと新schemaは互換ではありません。`DISCORD_ID_HMAC_KEY`と一時bootstrap IDを先にSecretへ登録し、注文受付停止・直前bookmark・adminがactiveな1人だけであることを確認してから、migration・新Worker・新Pagesを同じ停止時間内に切り替えます。admin再ログインと参加者ID再登録が終わるまで受付を再開しません。詳細は[データ移行](DATA_MIGRATION.md)に従います。
...
staging用のdeployment trigger commitをproduction履歴へ混ぜず、stagingで確認したrelease commitだけを`main`へ反映します。変更作業は`main`から作ったrelease branchへcommitし、そのbranchを`staging`へ通常mergeしてPreview確認します。`staging`とrelease branchのファイル内容が一致した後、`main`はrelease branchへfast-forwardします。

```powershell
$releaseBranch = "release/REPLACE_YYYYMMDD"
...
git diff --exit-code $releaseBranch staging --
git push origin staging
# Pages Previewとstaging実機確認がすべて成功してから続行する
git switch main
git pull --ff-only origin main
git merge --ff-only $releaseBranch
if ((git rev-parse HEAD) -ne $releaseSha) { throw "検証済みrelease SHAとmainが一致しません。" }
...
`REPLACE_YYYYMMDD`は実際のrelease日へ置換します。release branchは必ず最新`main`から作り、秘密情報検査・テスト・buildが成功した単一commitにします。`git status --short`が空であること、SHAと同一時点のJST/UTC時刻をrelease記録へ転記します。fast-forwardできない場合は`--force`や競合の自動解消をせず、作業を止めてGitHub Pull Requestで差分を確認します。Pull Requestを使った場合はmerge後に`main`を`git pull --ff-only`し、`git rev-parse HEAD`でSHAと検証時刻を取り直します。Pages dashboardでproduction build成功を待ち、表示されたcommit SHAが記録したSHAと完全一致すること、deployment IDと完了時刻を記録します。
...
管理画面から、本人確認済みの参加者・managerのDiscord User IDを1件ずつ事前登録します。登録済みアカウントのログイン、未登録アカウントの拒否、宴会コースの非表示、管理者代理追加と本人履歴を確認します。実注文はまだ作りません。

### 新D1の公開直前bookmarkを取る

管理者bootstrap・参加者のログイン許可登録・bootstrap Secret削除・注文を作らないsmoke testまで完了した状態を復元地点にします。
...
返されたbookmarkを「新D1公開直前bookmark」として、取得UTC/JST時刻、migration一覧、role別user件数、ログイン許可登録済み件数、menu件数と一緒にrelease記録へ保存します。ID・HMAC・Secret本体は記録しません。
...
推奨手順どおり管理者bootstrapと参加者登録後に取得した公開直前bookmarkなら、ログイン許可のHMACも復元されます。より古いbookmarkへ戻した場合は、adminのDiscord登録と参加者のログイン許可が失われます。その場合だけ、本人のDiscord User IDを`BOOTSTRAP_ADMIN_DISCORD_USER_ID`へ一時登録してadminを復旧し、成功直後に削除したうえで、参加者を管理画面から再登録します。
...
- Client Secret、HMAC鍵、bootstrap ID、実参加者名、Discord User ID、D1 bookmarkをチャットへ貼らない。
